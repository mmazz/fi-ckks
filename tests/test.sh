...
if cmp -s <(zcat /tmp/t0/data/*.gz) <(zcat /tmp/t1/data/*.gz); then
  echo "PASS add transitorio"
else
  echo "FAIL add transitorio"; exit 1
fi
for s in $(seq 0 25); do
  rm -rf /tmp/s
  build/bin/fi_heaan --stage mul --op_step $s --pipeline 'mul x2' $L --results_dir /tmp/s >/dev/null 2>&1 || echo "FALLA step $s"
done
